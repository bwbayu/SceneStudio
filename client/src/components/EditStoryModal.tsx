...
export default function EditStoryModal({ isOpen, onClose, story, onSave }: EditStoryModalProps) {
  ...
  const [previewImage] = useState(story?.thumbnail ?? '');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="glass relative w-full max-w-2xl animate-[scale-in] overflow-hidden rounded-2xl border border-border-default bg-bg-card shadow-2xl">
        ...
        <div className="absolute left-0 top-0 z-10">
          <div className="bg-linear-to-r ffrom-(--color-accent-primary) to-(--color-accent-secondary) px-5 py-2.5 shadow-lg">
        ...
        <div className="px-8 pb-10 pt-20">
          ...
          {/* Main Preview Box */}
          <div className="relative mb-10 flex aspect-video w-full items-center justify-center overflow-hidden rounded-xl border border-border-default bg-bg-secondary shadow-[inset_0_0_40px_rgba(0,0,0,0.4)]">
            {isGenerating ? (
              <div className="flex flex-col items-center gap-4">
                <div className="h-12 w-12 animate-spin rounded-full border-4 border-(--color-accent-primary) border-t-transparent" />
                <p className="text-sm font-medium text-text-secondary animate-pulse">Generating your masterpiece...</p>
              </div>
            ) : previewImage ? (
              ...
              <div className="flex flex-col items-center gap-4 text-center">
                ...
                <span className="text-xs font-bold uppercase tracking-widest text-text-muted">Preview Generated Story Thumbnail</span>
              </div>
            )}
            ...
            <div className="flex flex-col gap-4">
              <div className="flex flex-col overflow-hidden rounded-xl border border-border-default bg-bg-secondary transition-all duration-300 focus-within:border-(--color-accent-primary)/50 focus-within:shadow-[0_0_20px_rgba(200,164,90,0.05)]">
                <textarea
                  ...
                  className="w-full min-h-30 bg-transparent px-5 py-4 text-sm text-text-primary outline-none placeholder:text-text-muted resize-none"
                  style={{ fontFamily: "'Outfit', sans-serif" }}
                />
                <div className="flex justify-end items-center gap-3 border-t border-border-default bg-black/20 px-4 py-3">
                  {/* Generate Button */}
                  <button
                    onClick={handleGenerate}
                    disabled={isGenerating}
                    className="group relative flex items-center gap-2 overflow-hidden rounded-lg border border-border-default bg-white/5 px-6 py-3 text-[10px] font-bold uppercase tracking-[0.2em] text-text-secondary transition-all hover:bg-white/10 hover:text-text-primary active:scale-[0.98] disabled:opacity-50"
                  >
                    {isGenerating ? (
                      <>
