    ...
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
      ...
      <div className="glass relative w-full max-w-2xl animate-[scale-in] overflow-hidden rounded-2xl border border-[var(--color-border-default)] bg-[var(--color-bg-card)] shadow-2xl">
        ...
        <div className="absolute left-0 top-0 z-10">
          <div className="bg-gradient-to-r from-[var(--color-accent-primary)] to-[var(--color-accent-secondary)] px-5 py-2.5 shadow-lg flex items-center gap-2">
        ...
        <div className="px-8 pb-8 pt-20">
          ...
          <div className="relative mb-6 aspect-video w-full overflow-hidden rounded-xl border border-[var(--color-border-default)] bg-[var(--color-bg-secondary)] shadow-inner">
            {isGenerating ? (
              <div className="flex h-full flex-col items-center justify-center gap-4">
                ...
                <p className="text-sm font-medium text-[var(--color-text-secondary)] animate-pulse">Dreaming up your asset...</p>
              </div>
            ) : previewImage ? (
              ...
              <div className="flex h-full flex-col items-center justify-center gap-4 text-center p-10">
                <div className="rounded-full bg-white/5 p-5">
                   <MagicIcon className="h-10 w-10 text-[var(--color-text-muted)] opacity-50" />
                </div>
          ...
          {/* Prompt + Generate Row */}
          <div className="mb-8 flex items-center gap-3 rounded-xl border border-[var(--color-border-default)] bg-black/40 p-2 focus-within:border-[var(--color-accent-primary)]/50 transition-all">
            <input 
              type="text"
              value={prompt}
            ...
            <button
              ...
              className="rounded-lg bg-[var(--color-accent-primary)] px-6 py-2.5 text-[10px] font-black uppercase tracking-widest text-black transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50"
            >
              Generate
            </button>
          </div>
          ...
          <div className="flex justify-center">
            <button
              ...
              className="group relative flex items-center gap-2 overflow-hidden rounded-lg bg-gradient-to-r from-[var(--color-accent-primary)] to-[var(--color-accent-secondary)] px-12 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-black shadow-xl shadow-[var(--color-accent-primary)]/20 transition-all hover:scale-[1.05] hover:shadow-[var(--color-accent-primary)]/40 active:scale-[0.95] disabled:opacity-30 disabled:hover:scale-100"
            >
              Save Asset
        ...
        {/* Decorative Glows */}
        <div className="absolute -right-20 -top-20 -z-10 h-64 w-64 rounded-full bg-[var(--color-accent-primary)] opacity-[0.05] blur-[100px]" />
        <div className="absolute -bottom-20 -left-20 -z-10 h-64 w-64 rounded-full bg-[var(--color-accent-primary)] opacity-[0.05] blur-[100px]" />
      </div>
    </div>
  );
}
