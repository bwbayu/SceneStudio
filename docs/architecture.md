...
graph TD
    ...
    subgraph Gemini["Google Gemini Platform"]
        GFlash["gemini-2.5-flash-preview\nText Reasoning\n(Multi-Agent)"]
        GImage["gemini-2.0-flash-preview-image-generation\n/ nano-banana-2\nImage Generation"]
        GVeo["veo-3.0-generate-preview\n/ veo-3-1\nVideo Generation"]
    end
...
The backend uses **Google ADK** to orchestrate 7 specialized `LlmAgent` instances, all powered by `gemini-2.5-flash-preview`. Agents run sequentially or in parallel depending on their data dependencies.
...
sequenceDiagram
    ...
    participant GM as Gemini Image Model<br/>(gemini-2.0-flash-preview-image-generation<br/>/ nano-banana-2)
...
sequenceDiagram
    participant FE as Frontend (React)
    participant BE as Backend (FastAPI)
    participant Veo as Veo Model<br/>(veo-3.0-generate-preview / veo-3-1)
    participant GCS as Google Cloud Storage
    participant FS as Firestore
